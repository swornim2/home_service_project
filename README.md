...
```bash
# Register user
curl -X POST https://homeboundcare.preview.emergentagent.com/api/auth/register \
...
# Login
curl -X POST https://homeboundcare.preview.emergentagent.com/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "test@example.com",
    "password": "password123"
...
# Get services (use token from login)
curl -X GET https://homeboundcare.preview.emergentagent.com/api/services \
